...
public class RandomNumbers {
	...
	public static class Uniform {
		 ...
		 * @param nonNegative
		 *            if true, values may not be negative
		 */
		public Uniform(int min, int max, boolean nonNegative) {
		...
		/**
		 * Constructor for non-negative random numbers.
		 * 
		 * @param min
		 *            the minimal value to generate
		 * @param max
	...
	public static class Uniform2 {
		 ...
		 * @param min2
		 *            the minimum value of the first range, cannot be larger than min
		 * @param max2
		 *            the maximum value of the first range, cannot be smaller than max
		 */
	 ...
	 * burst.
	 * <li>The <b>large increments</b>. The difference between the last number in
	 * burst and the first number in the next burst.
	 * <li>The <b>base</b>. The first number.
	 * </ul>
	 * The first three properties are not numbers but ranges of a uniform random
	 * variable. For example a burst size of 1-5 specifies a random value in that
	 * range; an increment of 5000-10000 specifies a random value in that range
	 * (between 5 and 10 seconds when interpreting the numbers as milliseconds).
	 */
	public static class RandomBurst {
		...
		public RandomBurst() {
			super();
			randomLargeIncrement = new Uniform(10 * 60 * 1000, 100 * 60 * 1000); // 10-100 minutes 
			randomSmallIncrement = new Uniform(3 + 1000, 20 * 1000); // 3-20 seconds
			randomBurstSize = new Uniform(1, 5); // 1-5 events in a burst
		}
		
		/**
		 * Set the base. The default value is zero. If the argument is negative, the
		 * base will be computed on first access using
		 * <code>System.currentTimeMillis()</code>, the number of milliseconds since
		 * January 1970 in the UTC domain at the time of invocation.
		 * Note that if the first use of {@link #test} is also done with a negative
		 * value, the first test will always return true. To avoid this effect,
		 * make a test with a negative value and discard the result before making
		 * actual tests. 
		 * 
		 * @param base
		 *            a number
		...
		/**
		 * Set the range for the number of events during a burst. The actual number
		 * will be chosen at random in the range specified. The default values are 1
		 * and 5.
		 * <p>
		 * Passing two zero values disables generation of random numbers. In such a
		 * case {@link #next} will always return the current number and
		 * {@link #test} will always return false.
		 * 
		 * @param min
		...
		/**
		 * Set the range for the increment between two numbers during a burst. The
		 * actual increment will be chosen at random in the range specified. When
		...
		/**
		 * Set the range for the increment between two bursts. The actual increment
		 * will be chosen at random in the range specified. When interpreting the
		 * parameters as milliseconds, the default values correspond to 10 and 100
		 * minutes.
		 * 
		 * @param min
		...
		/**
		 * Return the next number in a non-decreasing sequence of positive random
		 * numbers. The first number returned is not smaller than the one last set
		 * with {@link #setBase} and it has a large increment from the base. It is
		 * followed by a random number of numbers with small increments. The next
		 * number has a large increment. And so on. If zero values were specified
		 * for small increments, there will be only large increments.
		 * <p>
		 * When random number generation is disabled (because of a zero burst size
		 * was specified), the method returns the unchanged current number.
		 * 
		 * @return a positive random number not smaller than the previous one or the
		 *         base if just set
		 */
		public long next() {
			if (currentNr < 0)
		 ...
		 * using <code>System.currentTimeMillis()</code>, the number of milliseconds
		 * since January 1970 in the UTC domain. Read the documentation of the
		 * {@link #setBase} method to understand what happens when using a negative
		 * value for both {@link #setBase} and {@link #test}.
		 * <p>
		 ...
		 * @param testValue
		 *            a number
		 * @return false if the argument is smaller than the current random number
		 *         or if random numbers are disabled
		 */
		...
		@Override
		public String toString() {
			return isDisabled() ? "burst size: 0 (disabled)" : 
					String.format("burst size: %s small increment: %s large increment: %s",
					randomBurstSize, randomSmallIncrement, randomLargeIncrement);
		}
